...
def extract_and_repeat(file_prefix, first_day, last_day, count):
    ...
    [(prec_data, "precipitation_amount", "Monthly precipitation amount", "kg m-2"),
     (wet_data, "number_of_days_with_lwe_thickness_of_precipitation_amount_above_threshold", "wet_days", "count")])

    export_data("{}_repeat_temp.nc".format(file_prefix), days, longitude, latitude, land_id,
    [(temp_data, "air_temperature", "Near surface air temperature at 2m", "K")])

    export_data("{}_repeat_rad.nc".format(file_prefix), days, longitude, latitude, land_id,
    [(rad_data, "surface_downwelling_shortwave_flux", "Mean daily surface incident shortwave radiation", "W m-2")])

def export_data(filename, days, longitude, latitude, land_id, data):
    pass

if __name__ == "__main__":
    extract_and_repeat("", 0.0, YEARS_100, 220)
