...
logging.info('Creating soil layer under bedrock layer with {}m thickness'.format(initialSoilDepth))
...
vegiTimeseries[ssntSF:] = vp - stepChangeVegi
...
DDdiff = DepthDependentVegiDiffuser(mg, alpha=alphaDiff, soil_creep_efficiency=soilCreepEfficienyBase)
...
t0 = time.time()
elapsed_time = 0
logging.info("starting with main loop.")
...
while elapsed_time < totalT:
    ...
    mean_K_diff.append(np.mean(DDdiff._kdv))
...
plt.close()
logging.info("FINALLY! TADA! IT IS DONE! LOOK AT ALL THE OUTPUT I MADE!!!!")
